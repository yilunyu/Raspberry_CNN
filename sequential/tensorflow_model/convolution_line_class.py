...
# tf Graph Input
x = tf.placeholder(tf.float32, [None,1,24, 32],name='x') # mnist data image of shape 28*28=784
y = tf.placeholder(tf.float32, [None, 3],name ='y') # 0-9 digits recognition => 10 classes

# Set model weights
filter_1 = tf.Variable(tf.zeros([24, 32, 1, 1]))
#b = tf.Variable(tf.zeros([3]))

# Construct model
#pred = tf.nn.softmax(tf.matmul(x, W) + b) # Softmax
stride = 1
conv_1_tmp = tf.nn.conv2d(x,filter_1,[1,1,stride,stride],padding="VALID",data_format="NCHW")

conv_1 = tf.nn.relu(conv_1_tmp)
pred = conv_1
...
with tf.Session() as sess:
    ...
    for f in os.listdir('/home/pi/tensorflow_example/artificial-processed'):
        ...
        img_data= np.reshape(img_data,[1,24,32])
        xs.append(img_data)
        fname = f.split('-')
        if('right' in fname[0]):
            ys.append([0,0,1])
